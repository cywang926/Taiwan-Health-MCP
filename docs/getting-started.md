...
FDA 藥品、健康食品、營養資料會在 MCP server **首次收到連線時**自動從 FDA API 同步，無需手動操作。

其他術語資料（ICD-10、LOINC、SNOMED CT、RxNorm、TWCore IG、臨床指引）需要：

1. 從官方來源申請並下載原始資料後放置到 `fhir-code/` 目錄（詳見 `fhir-code/README.md`）
2. 執行 data-loader：
...
儲存後重啟 Claude Desktop，即可在對話中使用所有 46 個醫療工具。
