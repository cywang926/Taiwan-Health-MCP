...
```json
{
  ...
  "services": {
    ...
    "twcore": true, "snomed": true, "drug_interactions": true
  }
}
```
...
FastMCP 在 streamable-http 模式下對每個 session 執行 lifespan。`metrics.start_metrics_server()` 已加入 `_metrics_server_started` flag 防止重複綁定。若仍出現此錯誤，請更新 `src/metrics.py`。
