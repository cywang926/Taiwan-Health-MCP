...
- **FastMCP lifespan-per-session**：`streamable-http` 模式下每個 MCP session 觸發一次 lifespan，但 `_init_lock + _initialized` 確保只有第一個 session 執行初始化，後續 session 重用已建立的連線池與 Redis。
...
## 併發處理
MCP Server 基於 FastMCP（uvicorn/asyncio）框架。若需處理大量請求：
