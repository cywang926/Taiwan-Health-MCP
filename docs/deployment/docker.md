...
其他術語資料（ICD、LOINC、SNOMED CT 等）需要先從官方來源合法取得，再手動放置原始檔案並執行 data-loader：

```bash
# 準備 fhir-code/ 目錄下的原始資料後（見 fhir-code/README.md）
...
```json
{
  "mcpServers": {
    "taiwan-health": {
      "command": "docker",
      "args": [
        "compose", "-f", "/path/to/Taiwan-Health-MCP/docker-compose.yml",
