...
FDA 藥品、健康食品、營養資料可透過 `data-loader --fda` 預先初始化；若 `--fda` 會包含藥品匯入，需先載入 `--rxnorm`。若未預先載入，app 在首次啟動或資料過期時也會自動從 FDA Open Data API 同步。
...
# 或只載入 FDA 動態資料
# 注意：Drug 具 RxNorm-first 防呆，未先載入 RxNorm 時會阻擋 --drug/--fda
...
> 若只需健康食品或營養資料，可單獨使用 `--health-food` 或 `--food-nutrition`，不受 RxNorm 前置限制。

### 5. 既有環境遷移（無資料遺失）

若資料庫是舊版（仍可能有 `rxnorm.*` 舊表或 drug 子表缺少完整約束），先執行：
...
此 migration 會：
- 將舊 `rxnorm.*` 資料合併到 `drug.rx_*`
- 先備份異常/重複列到 `migration_backup.*`，再去重
- 補上 FK / CHECK / NOT NULL / 索引，對齊 `db/schema.sql`
