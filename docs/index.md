...
本服務提供 **45 個 MCP 工具**，包含 `health_check` 與 44 個領域工具，主要分為 12 個群組；工具分類與 status page 範例由同一份 registry 生成，避免文件和實作分岔。
