...
## 技術架構
- **資料來源**：FHIR IG 套件（`package.tgz`），由 data-loader `--twcore` 或 Admin Console 的 IG 頁面匯入。相依套件（如 `hl7.terminology.r4`、`hl7.fhir.r4.core`）可額外綁定並各自以 package-scoped IG 索引，使跨系統的 ValueSet 綁定能展開出真實碼。
