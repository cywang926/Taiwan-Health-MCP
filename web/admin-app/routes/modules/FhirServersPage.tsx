...
const SECURITY_PROFILE_OPTIONS: {
  ...
  },
  {
    value: "oauth2",
    label: "OAuth 2.0",
    desc: "Standard OAuth 2.0 AS. Discovery via .well-known/oauth-authorization-server.",
  },
  {
    value: "smart",
    label: "SMART on FHIR",
    desc: "SMART discovery (.well-known/smart-configuration), SMART scopes, aud audience. Public client allowed.",
  },
  {
    value: "iua",
    label: "IHE IUA",
    desc: "IHE Internet User Authorization. Same RFC 8414 discovery as OAuth2, resource audience, JWT token. Confidential client — client_secret required.",
  },
];
...
// Static form section. The wizard already shows one step at a time, so the
// section is never collapsible — it just gives each step a titled, bordered
// container. (`defaultOpen` is accepted but ignored for call-site compatibility.)
function FormSection({
  ...
  return (
    <section className="fhir-section">
      <div className="fhir-section__summary">
        ...
        <>
          <div className="fhir-form-grid">
            <div className="fhir-form-wide">
              <small className="field-help">
                {form.auth_profile === "smart"
                  ? "SMART on FHIR: discovery via .well-known/smart-configuration, audience sent as `aud`, system/* scopes. Public client allowed (PKCE, no client_secret required)."
                  : form.auth_profile === "iua"
                    ? "IHE IUA: discovery via .well-known/oauth-authorization-server (RFC 8414, same as plain OAuth2), audience sent as `resource`, requested_token_type defaults to JWT. Confidential client only — client_secret is required."
                    : "Plain OAuth2: discovery via .well-known/oauth-authorization-server (RFC 8414), no IHE/SMART-specific framing."}
              </small>
            </div>
            {isClientCreds && (
            <label>
          ...
          <div className="fhir-form-grid">
            ...
            <label>
              ...
              <small className="field-help">
                {form.auth_profile === "smart"
                  ? "The SMART configuration document, served from the FHIR (resource) server base URL at .well-known/smart-configuration — not the /metadata CapabilityStatement. It advertises the authorization/token endpoints."
                  : "The authorization server's RFC 8414 metadata document at .well-known/oauth-authorization-server (OAuth2 and IUA share it) — served from the authorization server, not the FHIR resource server."}{" "}
                Leave blank to auto-derive it.
              </small>
