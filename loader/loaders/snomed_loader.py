...
def _load_concepts_from_zip(zf: zipfile.ZipFile) -> list[tuple]:
    ...
    stream = (
        _open_rf2(zf, r"Snapshot/Terminology/sct2_Concept_Snapshot_INT")
...
def _load_descriptions_from_zip(zf: zipfile.ZipFile) -> list[tuple]:
    ...
    stream = (
        _open_rf2(zf, r"Snapshot/Terminology/sct2_Description_Snapshot-en_INT")
...
def _load_relationships_from_zip(zf: zipfile.ZipFile) -> list[tuple]:
    ...
    stream = (
        _open_rf2(zf, r"Snapshot/Terminology/sct2_Relationship_Snapshot_INT")
...
def _load_icd10_map_from_zip(zf: zipfile.ZipFile) -> list[tuple]:
    ...
    stream = (
        _open_rf2(zf, r"Snapshot/Refset/Map/der2_iisssccRefset_ExtendedMapSnapshot_INT")
...
async def load_snomed(pool: asyncpg.Pool, zip_path: str) -> None:
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            # Clear existing data
            await conn.execute("TRUNCATE snomed.icd10_map, snomed.relationships, snomed.descriptions, snomed.concepts CASCADE")
            ...
            for c in concepts:
                etime_str = c[1]
                etime = date(int(etime_str[:4]), int(etime_str[4:6]), int(etime_str[6:8]))
