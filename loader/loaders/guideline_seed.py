...
async def seed_guidelines(pool: asyncpg.Pool) -> None:
    ...
    async with pool.acquire() as conn:
        ...
        gid = await conn.fetchval(
            ...
            "2024 台灣糖尿病臨床照護指引", "中華民國糖尿病學會", 2024,
        ...
        await conn.executemany(
            ...
            [
                (gid, "第一線", "雙胍類 (Biguanide)", "Metformin",
                 "起始劑量 500mg 每日一次，逐漸增加至 500-1000mg 每日兩次", "腎功能不全 (eGFR <30)", "A"),
                (gid, "第二線", "SGLT2 抑制劑", "Empagliflozin, Dapagliflozin",
                 "依藥品仿單建議劑量", "eGFR <20-30 (依藥品而異)", "A"),
                (gid, "第二線", "GLP-1 受體促效劑", "Dulaglutide, Semaglutide",
                 "皮下注射，每週一次", "甲狀腺髓樣癌病史或家族史", "A"),
                (gid, "第二線", "DPP-4 抑制劑", "Sitagliptin, Linagliptin",
                 "依藥品仿單建議劑量", "無特殊禁忌", "B"),
                (gid, "輔助治療", "胰島素", "基礎胰島素 (Insulin Glargine, Detemir)",
                 "依血糖監測調整劑量", "低血糖風險", "A"),
            ],
        )
        ...
        await conn.executemany(
            ...
            [
                ...
                (gid, "尿液檢驗", "尿液微量白蛋白", None, "每年至少1次", "早期腎病變偵測", "A"),
            ],
        )
        ...
        await conn.executemany(
            """INSERT INTO guideline.medication_recommendations
               ...
               VALUES ($1,$2,$3,$4,$5,$6,$7)""",
            [
                (gid, "第一線", "血管收縮素轉化酶抑制劑 (ACEI)", "Enalapril, Lisinopril",
                 "依血壓反應調整，從低劑量開始", "妊娠、雙側腎動脈狹窄、高血鉀", "A"),
                (gid, "第一線", "血管收縮素受體阻斷劑 (ARB)", "Losartan, Valsartan",
                 "依血壓反應調整", "妊娠、雙側腎動脈狹窄、高血鉀", "A"),
                (gid, "第一線", "鈣離子通道阻斷劑 (CCB)", "Amlodipine, Nifedipine",
                 "依血壓反應調整", "心衰竭（需謹慎）", "A"),
                (gid, "第一線", "利尿劑 (Thiazide)", "Hydrochlorothiazide, Indapamide",
                 "低劑量開始，監測電解質", "痛風病史（需謹慎）", "A"),
                (gid, "第二線", "乙型阻斷劑 (Beta-blocker)", "Bisoprolol, Carvedilol",
                 "有心臟病或年輕患者優先考慮", "氣喘、心搏過慢", "B"),
            ],
        )
        ...
        await conn.executemany(
            ...
            [
                ...
                (gid, "心電圖", "靜態心電圖", None, "診斷時及每年追蹤", "評估心臟肥大或缺血", "B"),
        ...
        await conn.execute(
            """INSERT INTO guideline.treatment_goals
               ...
               VALUES ($1,$2,$3,$4,$5)""",
            gid, "血壓控制", "血壓",
            "<140/90 mmHg（一般成人），<130/80 mmHg（糖尿病或慢性腎臟病患者）", "長期目標",
        )
        ...
        gid = await conn.fetchval(
            """INSERT INTO guideline.disease_guidelines
               (icd_code, disease_name_zh, disease_name_en, guideline_title,
                guideline_source, publication_year, guideline_summary)
               VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id""",
            "E78", "高血脂症", "Hyperlipidemia",
            "2023 台灣血脂異常治療指引", "中華民國血脂及動脈硬化學會", 2023,
        ...
        await conn.executemany(
            """INSERT INTO guideline.medication_recommendations
               ...
               VALUES ($1,$2,$3,$4,$5,$6,$7)""",
            [
                (gid, "第一線", "史他汀類 (Statin)", "Atorvastatin, Rosuvastatin",
                 "中至高強度，依心血管風險決定", "活動性肝病、孕婦", "A"),
                (gid, "第二線", "Ezetimibe", "Ezetimibe",
                 "10mg 每日一次，可與 Statin 併用", "無特殊禁忌", "B"),
                (gid, "第二線", "PCSK9 抑制劑", "Evolocumab, Alirocumab",
                 "皮下注射，用於高風險且 Statin 無法達標者", "成本考量", "A"),
                (gid, "其他", "纖維酸類 (Fibrate)", "Fenofibrate",
                 "主要用於高三酸甘油酯", "腎功能不全", "B"),
            ],
        )
        ...
        await conn.execute(
            """INSERT INTO guideline.test_recommendations
               (guideline_id, test_category, test_name, loinc_code,
                frequency, indication, evidence_level)
               VALUES ($1,$2,$3,$4,$5,$6,$7)""",
            gid, "生化檢驗", "血脂肪 (TC, LDL, HDL, TG)", "2093-3",
            "開始治療前及治療4-12週後追蹤", "評估治療效果", "A",
        )
        ...
        await conn.executemany(
            ...
            [
                (gid, "血脂控制", "LDL-C",
                 "<100 mg/dL（中風險），<70 mg/dL（高風險），<55 mg/dL（極高風險）", "依心血管風險分層"),
        ...
        gid = await conn.fetchval(
            ...
            "2023 台灣慢性腎臟病臨床診療指引", "台灣腎臟醫學會", 2023,
        ...
        await conn.executemany(
            ...
            [
                ...
                (gid, "尿液檢驗", "尿液白蛋白肌酸酐比值 (UACR)", None, "每年", "蛋白尿監測", "A"),
